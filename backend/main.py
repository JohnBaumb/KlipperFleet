...
app = FastAPI(
    title='KlipperFleet API', version='1.2.0-alpha', lifespan=lifespan
)
    ...
    async def run_task() -> None:
        ...
        try:
            ...
            if 'build' in action:
                ...
                profiles_to_build: List[Any] = list(
                    set(d['profile'] for d in devices if d['profile'])
                )
                if not profiles_to_build:
                    ...
                    for profile in profiles_to_build:
                        ...
                        task_store.add_log(
                            task_id, f'>>> BATCH BUILD: Finished {profile}\n'
                        )
