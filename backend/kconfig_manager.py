...
class KconfigManager:
    ...
    def _run_firmware_extras_script(self, klipper_dir: str) -> None:
        ...
        Kalico (Danger Klipper) adds a src/extras/ directory for user firmware extensions.
        The src/Kconfig references src/extras/Kconfig via a 'source' directive, but that
        file is auto-generated by scripts/find-firmware-extras.sh during 'make'. Since
        KlipperFleet bypasses make, we need to run the script ourselves.
        """
        extras_script = os.path.join(klipper_dir, "scripts", "find-firmware-extras.sh")
        extras_kconfig = os.path.join(klipper_dir, "src", "extras", "Kconfig")
        
        if not os.path.exists(extras_script):
            return
